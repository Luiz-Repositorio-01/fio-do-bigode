...
export function Location() {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
      <div className="space-y-6 rounded-md border border-edge bg-panel p-5 md:p-8">
            ...
            <dl className="mt-3 space-y-1.5 text-[15px]">
              {schedule.map((row) => (
                <div key={row.label} className="flex justify-between gap-4 border-b border-edge/60 pb-1.5">
                  ...
                  <dd className="whitespace-nowrap tabular-nums">{row.value.replace(" às ", " – ")}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row">
          <AnchorButton href={mapsUrl} target="_blank" rel="noopener noreferrer" className="sm:flex-1">
      ...
      <div className="min-h-64 overflow-hidden md:min-h-80 rounded-md border border-edge bg-wood">
        <iframe
          title={`Mapa: ${fullAddress}`}
          src={mapsEmbedUrl(fullAddress)}
          loading="lazy"
          referrerPolicy="no-referrer-when-downgrade"
          className="h-full min-h-64 w-full md:min-h-80 border-0 [filter:grayscale(.55)_contrast(1.05)_invert(.92)_hue-rotate(180deg)]"
        />
      </div>
    </div>
  );
